...
<#
    ...
    .DESCRIPTION
        This function imports data from a CSV file into a specified SQLite database table. The function sorts the data by date before importing it into the table. This updated version of the function includes additional fields for the imported data.
...
# Push-ToSQLite -DBPath "$CPSScriptRoot\Export\CHData.sqlite" -CSVPath "$CPSScriptRoot\Export\CHData.csv" -SQLitePath "$CPSScriptRoot\Functions\SQLite\SQLite.Interop.dll" -CPSScriptRoot $CPSScriptRoot -TableName 'CHData'
